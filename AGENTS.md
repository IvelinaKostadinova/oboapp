...
- **Reference screenshots (REQUIRED):** Every crawler directory must include visual baseline screenshots of the source pages used by the crawler.
  - Save at least one entry/listing screenshot and one message/detail screenshot when the source has both page types.
  - Preferred naming: `_entry.png` and `_message.png` (source-specific names are allowed when structure differs).
  - Store screenshots directly in `ingest/crawlers/{source-name}/`.
  - Update screenshots when site design changes and selectors/parsers are adjusted.
